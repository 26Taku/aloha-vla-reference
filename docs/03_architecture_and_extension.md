...
本資料は、**ALOHAへcamera・F/T・IMU・tactile sensor等を追加し、robot dataと同時収録して検証するまでの操作・設計マニュアル**である。
...
1. **任意sensorへ適用する共通手順**
   - device / stream確認
   - acquisition方式の選択
   - raw dataとtimestampの保存
   - ALOHAとのconcurrent acquisition
   - causal alignment
   - validation
2. **実機で確認した具体例**
   - high-rate numeric sensor: MMS101を用いたROS 2 stream
   - asynchronous camera: GelSight Miniを用いたV4L2 stream

MMS101やGelSight Miniを使用しない場合も、共通手順を対象sensorのdriver/interfaceへ適用する。
...
## 2. Sensor extensionの完了条件

sensor extensionは、sensor単体が読めた時点では完了としない。以下をすべて確認する。

```text
Baseline ALOHA -> READY / PASS
        ↓
Sensor device / streamを確認
        ↓
Sensor単体取得
        ↓
...
Pattern B / Cでは、sensor driver自体を本repositoryへ統一しない。ROS 2、V4L2、serial API、vendor SDK等、対象deviceが提供するinterfaceでraw dataを取得し、本referenceのtimestamp / alignment contractへ接続する。

---

## 4. 共通data / timestamp contract

### 4.1 Canonical raw data
...
この節はsensor固有driverが起動し、numeric streamを取得できるinterfaceがあることを前提とする。本repositoryはgeneric ROS 2 numeric loggerをreference adapterとして提供する。

### 6.1 Sensor driverを起動する

対象sensorのdriverを起動する。

ROS 2 driverを使用する場合、driverをsourceしたshellで:
...
`ros2 topic hz` は数秒以上観測してrateが安定していることを確認する。
...
`receive_monotonic_ns` がhost側alignment用timestampである。
...
研究室で使用したMMS101はROS 2 driverからnumeric time-seriesとして取得した。

この節はMMS101専用driverを本repositoryが提供するという意味ではなく、Section 6の具体的な適用例である。

### 7.1 Driver availability

ROS 2 environmentをsourceし、packageが見えることを確認する。

```bash
ros2 pkg prefix mms101_driver
```

validated environmentではdual launch fileを使用した。

```bash
ros2 launch mms101_driver mms101_dual.launch.py
```

launch argumentを確認する場合:

```bash
ros2 launch mms101_driver mms101_dual.launch.py --show-args
```

validated launch configurationのdefault device aliases:
...
device aliasが環境に存在することを確認する。

```bash
ls -l /dev/mms101_left /dev/mms101_right
```

aliasを使用しない環境では、接続deviceを確認した上でlaunch fileのport設定を対象環境へ合わせる。

### 7.2 Topics

validated configurationのtopics:
...
```bash
ros2 topic list | grep force_torque

ros2 topic type /force_torque/left
ros2 topic type /force_torque/right
```

MMS101 node実装では `geometry_msgs/msg/WrenchStamped` をpublishする。
...
rateはdriver/configurationに依存するため、固定値を仮定せず実測する。
...
python3 examples/custom_sensor/ros2_timeseries_logger.py \
  --topic /force_torque/left \
  --msg-type geometry_msgs/msg/WrenchStamped \
  --sensor-id mms101_left \
  --output data/_sensor_runs/mms101_left.jsonl \
  --duration 10
```
...
```bash
python3 examples/custom_sensor/ros2_timeseries_logger.py \
  --topic /force_torque/right \
  --msg-type geometry_msgs/msg/WrenchStamped \
  --sensor-id mms101_right \
  --output data/_sensor_runs/mms101_right.jsonl \
  --duration 10
```

dual sensorをALOHAと同時取得する場合は、左右loggerを別processとしてrobot recordingより先に起動する。

過去のreference validationではnative-rate acquisition、ALOHA concurrent recording、latest-value alignment、200 ms history-window generationまで確認している。実測値は [06 Validation Results](06_validation_results.md) を参照する。
...
本referenceではLinux V4L2で認識されるcameraを具体例とする。
...
不足している場合は対象OSのpackage manager等で導入する。

### 8.2 Camera deviceを列挙
...
対象cameraのcandidate deviceを決める。

### 8.3 Format / resolution / FPSを確認
...
使用するpixel format / resolution / FPSはSection 8.3で対象cameraについて確認した値へ置き換える。
...
```text
[ ] V4L2でtarget cameraを識別
[ ] format / resolution / advertised FPSを確認
[ ] MKV captureが完了
[ ] ffprobeでvideoを読める
...
別個体・firmware・hostでは再度 `--list-formats-ext` で確認する。
...
過去のreference validationではactual rateは約18.7--18.8 Hzだった。advertised FPSではなくtimestampからactual rateを確認する。
...
ALOHAと同時取得する場合はSection 8.6--8.8をそのまま実行する。

過去のreference validationでは、GelSight Mini 1台についてnative compressed acquisition、packet timestamp保持、ALOHA concurrent recording、causal latest-frame alignmentまで確認している。実測値は [06 Validation Results](06_validation_results.md) を参照する。
